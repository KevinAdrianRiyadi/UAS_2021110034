...
<body>
    <div class="container mt-5">
        ...
        <form action="{{ route('updatekitchen', $data->id) }}" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="mb-3">
                <label for="name" class="form-label">Nama kitchen</label>
                <input type="text" class="form-control" id="nama" name="nama" required
                value="{{$data->nama}}">
                
            </div>
            <div class="mb-3">
                <label for="name" class="form-label">Kategori kitchen</label>
